...
of `r d[1]` rows and `r d[2]` columns.<br>
...
the customers who switched to a different provider represent only the `r rounding_two_decimals_percentage(103/500)`% of our sample.
An unbalanced data set can affect our model performance. Models trained on unbalanced 
...
webget_hist <- plot_gghistogram(x=webget, x_lab='Mean time to load a webpage', title_ ='Webpage mean loading time distribution')
enqcount_hist <- plot_gghistogram(x=enqcount, x_lab='Enquiry count', title_ = 'Enqcount distribution')
callwait_hist <- plot_gghistogram(x=callwait, x_lab='Waited time', title_ ='Waiting time to be attended by customer service distribution')
...
```
In order to produce a classification model, we firstly split our data set into a 
...
Firstly we are going to use the KNN classifier algorithm to make predictions.
...
the values are assigned as no, resulting 31 false negatives for k = 1, thus a recall of
of only `r rounding_two_decimals_percentage(3/(3+31))`%.<br>That is an issue resulted from the imbalance of our set.
...
abline(h = min(cross_knn_error), col = "darkorange", lty = 3)
```
...
that for $k=1$ the model predicts better the true positives (those who switched providers) resulting a recall `r rounding_two_decimals_percentage(42/(42+27))`,
...
*Mean decrease Gini* shows the total decrease in node impurity gained resulted by the splits
of a given variable, averaged over all trees. If a variable is useful. it tends to split a node into pure single class nodes. Thus, removing such variable, results a decrease in the average purity gained resulted by this variable. 
...
kable(table(bag_tree_pred, df_test_tele[,5]),
...
while for rf test error is only `r rounding_two_decimals_percentage(bag_tree_train_error)`%, and thus its accuracy is `r rounding_two_decimals_percentage(bag_tree_train_acc)`%.
We notice that the rf performed much better predicting those who switched providers (True Positives), having a recall of `r rounding_two_decimals_percentage(29/(29+5))`%, compared to LOOCV KNN with recall = `r rounding_two_decimals_percentage(33/(33+36))`.
In general both KNN models, scored very poorly, on predicting those who switched providers. That derives from the unbalance of our dataset.
...
```
We notice that the **biplot** function does not produce as clean plot, to this end
...
1. The more parallel to a principal component axis is a vector, the more it contributes to that principal component. 
Thus, the variables **upload** and **webget** have very high (positive) contribution on the first principal component (the highest contribution is due to **webget**), while the variables **enqcount** and **callwait** appear not to contribute at all.

2. On the other hand, the variables **enqcount** and **callwait** have very high (negative) contribution on the second principal component,  
while the variables **upload** and **webget** appear not to contribute at all.

3. The longer the vector, the more variability of this variable is represented by the two displayed principal components. The variables  **upload** and **webget** are longer than the **enqcount** and **callwait**, indicating that **upload** and **webget** that more variability of those variables is presented by these two principal components, compared to the other two variables.

4. When two vectors form a small angle between them, it represents that the two variables are positively correlated. Thus, the pair **upload** - **webget**, 
and the pair **enqcount** - **callwait** are positively correlated. However the variables those two pairs have no correlation between them, since their respective vectors, form about a 90 degrees angle. 
...
kable(table(comp_forest_pred, comp_df_test[,3]),
...
which is worse than the test error obtained from the random forest which used the 
bagging method (test error `r rounding_two_decimals_percentage(bag_tree_train_error)`%).

In general we notice that, in this case the algorithm performs poorer in predicting the True Positives and True Negatives.
However, if we take under consideration the the majority of the churn values are 'no', we notice that by this approach the model,
seems to not get affected by the majority class. It has the lowest recall for the 'No' class compared to the rest of our models (`r rounding_two_decimals_percentage(98/(98+18))`%)
...
while the second is colored by the actual class.
