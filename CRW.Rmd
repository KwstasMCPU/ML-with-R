...
the customers who switched to a different provider represent only the `r 103/500*100`% of our sample.
...
knn_7 <- knn(train = df_train_tele_norm[,-5], 
                test = df_test_tele_norm[,-5], 
                cl=df_train_tele_norm$churn, 
                k=7)

knn_7_accuracy <- mean(knn_7 == df_test_tele_norm$churn)
knn_7_error <- mean(knn_7 != df_test_tele_norm$churn)

knn_1_conf_matrix <- table(knn_1, df_test_tele_norm$churn)
knn_7_conf_matrix <- table(knn_7, df_test_tele_norm$churn)
...
rownames(knn_7_conf_matrix) <- c("No","Yes")
kable(knn_7_conf_matrix,
       col.names = c('No','Yes'),
      caption = ' Confusion Matrix for k=7')
```
We notice that for the both values of k, the algorithm struggles to predict the
correct the those who switched provider (true positives), while the majority of 
the values are assigned as no, resulting 31 false negatives for k = 1, and 32 for
k = 7.<br>That is an issue resulted from the imbalance of our set.

For both values of k, the test error is `r rounding_two_decimals_percentage(knn_1_error)`%. However, despite the fact 
that for $k=1$ the model predicts better the true positives (those who switched providers),
fewer neighbors corresponds to high model complexity (Müller & Guido, 2017), 
...
We notice that the random forest performs better compared to the KNN algorithm. The test error for KNN is `r rounding_two_decimals_percentage(knn_7_error)`%,
while for rf test error is only `r rounding_two_decimals_percentage(bag_tree_train_error)`%, and thus is accuracy `r rounding_two_decimals_percentage(bag_tree_train_acc)`%.
We notice that the rf performed much better predicting those who switched providers (True Positives), having a recall of `r rounding_two_decimals_percentage(29/(29+5))`%.
The KNN scored very poorly, on predicting those who switched providers, predicting correctly
only 3 out of the 34, resulting a recall of only `r rounding_two_decimals_percentage(2/(2+32))`%.
...
The test OOB error of random forest is `r rounding_two_decimals_percentage(comp_forest_error)`%.
