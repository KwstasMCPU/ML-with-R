...
Since we have variables with different scales, we should scale the data. Also since we are using KNN which uses the Euclidian distance,
...
knn_tele_normalized <- knn( train = norm_x_train, test = norm_x_test, cl = norm_y_train, k = 1)
```
...
norm_knn_accuracy <- mean(knn_tele_normalized == norm_y_test)
norm_knn_accuracy
```
