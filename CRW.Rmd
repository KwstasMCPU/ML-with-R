...
that the customers who switched provider represent only the `r 103/500*100`% of our sample.
...
For both values of k, the test error is `r knn_1_error`. However, despite the fact 
...
The test error is only `r bag_tree_train_error`, and thus is accuracy `r bag_tree_train_acc`.
We notice performed much better on the True Positives.
The KNN scored very poorly, on predicting those who switched provider, predicting correctly
only 3 out of the 34, resulting a recall of only `r 3/(3+31)`.
