...
For both values of k, the test error is `r knn_1_error`, however we prefer the value 
for k=1, since predicts better true positives (those who switched providers).
...
The better accuracy comes from the fact that scored better on the True Positives.
The KNN scored very poorly, on predicting those who switched provider, predicting correctly
, resulting a recall of only `r 3/(3+31)`. 
This stems from the fact that our data set is unbalanced.
